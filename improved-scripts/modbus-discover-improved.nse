...
-- @args modbus-discover.timeout - timeout in milliseconds (default: stdnse.get_timeout or 2000)
-- @args modbus-discover.sid-start - first SID to scan (default: 1)
-- @args modbus-discover.sid-end - last SID to scan (default: 246)
--
-- @output
-- PORT    STATE SERVICE
-- 502/tcp open  modbus
...
-- Version 0.4 - /11.07.26/ - configurable timeout, SID range args, improved non-aggressive mode
...
action = function(host, port)
  ...
  -- Configurable timeout: use modbus-discover.timeout arg, or fall back to stdnse.get_timeout
  local timeout = stdnse.get_script_args('modbus-discover.timeout')
  if ( not timeout ) then
    timeout = stdnse.get_timeout(host)
  else
    timeout = tonumber(timeout)
  end
  local opts = {request_timeout=timeout}

  -- Configurable SID range
  local sid_start = tonumber(stdnse.get_script_args('modbus-discover.sid-start') or 1)
  local sid_end = tonumber(stdnse.get_script_args('modbus-discover.sid-end') or 246)

  local results = stdnse.output_table()

  -- In non-aggressive mode, scan only 10 consecutive SIDs (starting from sid_start)
  -- and stop at the first that responds. This is truly "non-aggressive."
  local sid_upper_bound
  if aggressive then
    sid_upper_bound = sid_end
  else
    sid_upper_bound = math.min(sid_start + 9, sid_end)
  end

  for sid = sid_start, sid_upper_bound do
