...
id=i:1
  ...
  -- receive response
  -- Use explicit timeout via stdnse.get_timeout. comm.tryssl uses the
  -- default socket timeout if no option is provided; passing timeout
  -- here ensures the script respects --script-timeout and per-host
  -- timing overrides.
  local socket, response, proto = comm.tryssl(host, port, orig_query, {timeout=stdnse.get_timeout(host)})
  if not socket then
