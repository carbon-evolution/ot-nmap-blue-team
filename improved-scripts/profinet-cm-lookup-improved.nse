...
--|   annotationOffset: 0
--|   annotationLength: 64
--|   annotation: S7-1500                   6ES7 672-5DC01-0YA0      0 V  2  1  7
--|_  deviceName: S7-1500
-- @xmloutput
...
--<elem key="deviceName">S7-1500</elem>
...
---
-- Attempts to extract a device name from the annotation string.
-- PROFINET annotation fields typically contain the device name as the first
-- space-delimited token (e.g. "S7-1500" from "S7-1500  6ES7 ...").
---
local function extract_device_name(annotation)
  if not annotation or #annotation == 0 then
    return nil
  end
  -- Strip leading/trailing whitespace and grab the first token
  local name = annotation:match("^%s*(%S+)")
...
parse_response = function(host, port, layer3)
  ...
  local pkt_len = #layer3

  -- Minimum length needed to parse all fixed fields up through annotation payload.
  -- annotationOffset starts at byte 165 (1-indexed: needs 168 bytes for I4),
  -- annotationLength at 169 (needs 172), annotation data at 173.
  local MIN_PARSE_LEN = 173

  if pkt_len < MIN_PARSE_LEN then
    stdnse.debug(1, "Response too short to parse (%d bytes, need at least %d)", pkt_len, MIN_PARSE_LEN)
    return nil
  end
  ...
  if pkt_len < 173 then
    stdnse.debug(1, "Response too short for annotationLength field (%d bytes, need 173)", pkt_len)
    return nil
  end
  ...
  -- parse annotation
  if annotationLength <= 0 then
    stdnse.debug(1, "Annotation length is non-positive (%d), skipping annotation parse", annotationLength)
    return nil
  end

  if pkt_len < 173 + annotationLength then
    stdnse.debug(1, "Response too short for annotation payload (%d bytes, need %d for %d-byte annotation)",
      pkt_len, 173 + annotationLength, annotationLength)
    return nil
  end
  ...
  -- extract device name from annotation if present
  local deviceName = extract_device_name(annotation)
  if deviceName then
    output["deviceName"] = deviceName
    stdnse.debug(1, "deviceName: %s", deviceName)
  end
